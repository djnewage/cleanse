...
class TestCorrectWordsWithPlainLyrics:
    """Plain-lyrics (no timestamps) word-text correction via global alignment."""

    LYRICS = ("walking down the street tonight with nothing left to lose "
              "gotta keep it out the reach of kids counting every heartbeat")

    def _words(self, texts, conf=0.4, t0=10.0):
        return [{"word": t, "start": t0 + i * 0.5, "end": t0 + i * 0.5 + 0.4,
                 "confidence": conf, "is_profanity": False} for i, t in enumerate(texts)]

    def test_low_conf_mishears_corrected(self):
        from lyrics_corrector import correct_words_with_plain_lyrics
        # 'rich' misheard for 'reach' at low confidence — the screenshot case
        heard = ("walking down the street tonight with nothing left to lose "
                 "gotta keep it out the rich of kids counting every heartbeat").split()
        result = correct_words_with_plain_lyrics(self._words(heard), self.LYRICS)
        fixed = next(w for w in result if w.get("original_word") == "rich")
        assert fixed["word"] == "reach"
        assert fixed["detection_source"] == "lyrics_corrected"
        assert 0 < fixed["correction_confidence"] <= 1

    def test_high_conf_words_left_alone(self):
        from lyrics_corrector import correct_words_with_plain_lyrics
        heard = self.LYRICS.split()
        heard[heard.index("reach")] = "rich"
        result = correct_words_with_plain_lyrics(self._words(heard, conf=0.95), self.LYRICS)
        # conf >= 0.7 requires similarity >= 0.90; rich~reach is below that
        assert all("original_word" not in w for w in result)

    def test_wrong_song_lyrics_correct_nothing(self):
        from lyrics_corrector import correct_words_with_plain_lyrics
        rap = ("and if you hate that i don't spit out money and all the stuff i get "
               "junkie and y'all done bumped my tape keep it going all night long").split()
        punk = ("you don't like the clothes i wear i shave my head or grow my hair "
                "what makes you look over here what are you queer skins and bangers "
                "joining fight as one those who persecute battle til they have won")
        result = correct_words_with_plain_lyrics(self._words(rap), punk)
        assert all("original_word" not in w for w in result)

    def test_empty_inputs_unchanged(self):
        from lyrics_corrector import correct_words_with_plain_lyrics
        words = self._words(["hi"])
        assert correct_words_with_plain_lyrics(words, "") is words
        assert correct_words_with_plain_lyrics([], self.LYRICS) == []
